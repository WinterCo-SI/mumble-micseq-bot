...
在标记里追加 `/CC <频道ID>,<频道ID>`，机器人发到本频道的所有消息都会同时发一份到这些频道（私信不会）：

```
会议室 [麦序/发言时间: 300秒/下麦转12/CC 13,14]
```

- 频道 ID 之间可以用 `,`、`，`、`、` 或空格分隔。`CC` 不区分大小写，后面可以带冒号。
- 不存在的频道会被跳过，机器人会在频道里提示一次。
- 机器人在 CC 频道里也需要 TextMessage 权限。CC 频道的消息单独发送，某个 CC 频道没有权限不会影响本频道的消息。
- 各段顺序不限，`发言时间` 和 `下麦转` 必填。
...
- **提示消息**：所有消息都用表格卡片排版（Qt 富文本），中英双语。标题栏写明频道名，所以抄送到其他频道时也能看出是哪个频道的消息。
  - **频道消息**：发言人或队列有任何变化（有人加入、离开、轮到下一位等）时，立即在频道里发送最新的发言人和排队名单。同一事件产生的提示和名单合并成一条消息，以节省服务器的消息限额。没有变化时，每 `queue_announce_interval` 再发一次名单。
  - **私信**：只发给相关的人，不在频道里播报。
    - 加入队列时，告诉他排在第几位。接管频道时已经在频道里的人不单独发私信，频道里的名单已经列出了他们。
    - 轮到发言时，告诉他发言时长和结束后转到哪个频道。
    - 发言人按 `speaker_reminders` 收到剩余时间提醒。提醒按阶梯安排，时间充裕时间隔长，越接近结束越密（默认剩余 1 小时、30、15、10、5、3、2、1 分钟、30 秒、10 秒时各提醒一次）。
    - 下一位按 `next_speaker_reminders` 收到「准备发言」提醒（默认当前发言剩余 10、5、2、1 分钟、30 秒、10 秒时）。刚成为下一位时，如果剩余时间已经少于最大的一档，会立即提醒一次。
    - 时间到、被移到下麦频道时，告诉他移到了哪里。
- **同一用户多个会话**：同一个注册用户重新连接时，服务器可能会短暂保留旧会话（ghost），频道里就会出现两个相同 user_id 的会话。机器人把它们当作同一个人：只排一次队，私信只发给最新的会话。只要还有一个会话留在频道里，就不算离开或离线，不会触发离线等待和补偿。轮到他发言时，所有会话都会被解除禁言；发言结束时，所有会话都会被移走（下麦失败时全部禁言）。
- **ACL 被修改**：Mumble 服务器在任何 ACL 修改后都会重新禁言所有人，其中也包括正在发言的人。机器人会自动重新解除他的禁言。
- **下麦失败**：下麦频道不存在、已满或机器人没有 Move 权限时，改为禁言（mute）该发言人，并在频道里提示管理员、私信告诉本人。这个人离开频道时，机器人会解除这个禁言。
...
| `queue_announce_interval` | `60s` | 名单没有变化时，多久重发一次，`"off"` 表示关闭（有变化时总是立即发送） |
| `speaker_reminders` | `["1小时", "30分钟", "15分钟", "10分钟", "5分钟", "3分钟", "2分钟", "1分钟", "30秒", "10秒"]` | 剩余多少时间时私信提醒发言人，`"off"` 或 `[]` 表示关闭 |
| `next_speaker_reminders` | `["10分钟", "5分钟", "2分钟", "1分钟", "30秒", "10秒"]` | 当前发言剩余多少时间时私信提醒下一位，`"off"` 或 `[]` 表示关闭 |
...
旧版本的 `remaining_announce_interval` 和 `warn_before` 已经不再使用。配置文件里保留它们也能正常启动，但会在日志里提示，请改用 `speaker_reminders` / `next_speaker_reminders`。
